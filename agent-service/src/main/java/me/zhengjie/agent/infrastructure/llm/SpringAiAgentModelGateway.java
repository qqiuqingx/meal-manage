...
public class SpringAiAgentModelGateway implements AgentModelGateway {
    ...
    public boolean isConfigured(String profile) { return builder != null && properties.getModels().getProfiles().containsKey(resolve(profile)); }
    public ModelProfile profile(String profile) {
        String id = resolve(profile);
    ...
    private String resolve(String profile) {
        String requested = profile == null || profile.isBlank() ? "default" : profile;
        return properties.getModels().getProfiles().containsKey(requested) ? requested : "default";
    }
}
