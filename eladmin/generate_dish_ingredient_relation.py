        ...
        for idx, row in data_rows.iterrows():
            ...
            for name_col, ingredient_col, day in day_columns:
                ...
                for ingredient_name in ingredient_names:
                    ...
                    ingredient_name_escaped = ingredient_name.replace("'", "''")

                    # 生成关系SQL - 使用菜品名称和排期来匹配
                    sql = f"""INSERT INTO dish_ingredient_relation (dish_id, ingredient_id, create_time)
SELECT d.id, di.id, NOW()
FROM dish d, dish_ingredient di
WHERE d.name = '{dish_name_escaped}'
  AND JSON_CONTAINS(d.schedule, '"{schedule}"')
  AND di.name = '{ingredient_name_escaped}';"""

                    relation_sql_list.append(sql)

    # 写入SQL文件
    output_file = '/Users/qqx/job/code/eladmin-mp/eladmin/sql/insert_dish_ingredient_relations.sql'
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write("-- 菜品配料关系表数据插入SQL\n")
        ...
        for sql in relation_sql_list:
            f.write(sql + "\n")

    print(f"\n✅ 关系SQL生成完成！")
    print(f"📊 统计信息：")
    print(f"   - 关系记录数量: {len(relation_sql_list)}")
    print(f"   - 输出文件: {output_file}")

if __name__ == '__main__':
    generate_relation_sql()
