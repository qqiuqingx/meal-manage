...
Notes:
- The `dev` profile reads MySQL/Redis from environment variables in `eladmin-system/src/main/resources/config/application-dev.yml`; sandboxed runs may fail with `Operation not permitted (connect failed)`.
