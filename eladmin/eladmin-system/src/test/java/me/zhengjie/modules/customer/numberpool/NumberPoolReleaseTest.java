...
import me.zhengjie.modules.customer.profile.service.impl.CustomerProfileServiceImpl;
...
public class NumberPoolReleaseTest {
    ...
    public void testRelease_onCancel_reusesSmallestCode() {
        // Step 1: Create first customer + profile (has no code yet)
        CustomerProfile c1 = new CustomerProfile();
        c1.setCustomerName("Cancel-Test-Customer-1");
        c1.setPhone("13800001111");
        c1.setParentPackageId(testPackageId);
        c1.setStatus(true);
        profileMapper.insert(c1);

        // Step 2: Allocate code to first customer
        String code1 = profileService.generateCode(testPackageId);
        assertNotNull(code1);
        ...
        // Step 3: Create an ACTIVE order for customer 1 (status=1 holds the code)
        ...
        // Step 4: Cancel the order (status 1→0) — THIS IS THE RELEASE TRIGGER
        CustomerOrder activeOrderEntity = orderMapper.selectList(null).get(0);
        ...
        // Step 5: Create second customer
        CustomerProfile c2 = new CustomerProfile();
        c2.setCustomerName("Cancel-Test-Customer-2");
        c2.setPhone("13800002222");
        c2.setParentPackageId(testPackageId);
        c2.setStatus(true);
        profileMapper.insert(c2);

        // Step 6: Allocate code to second customer
        String code2 = profileService.generateCode(testPackageId);
    ...
    public void testRelease_multipleCancelled_smallestFirstReuse() {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            c.setParentPackageId(testPackageId);
            c.setStatus(true);
            profileMapper.insert(c);
            customerIds[i] = c.getId();
            codes[i] = profileService.generateCode(testPackageId);
        ...
        c4.setParentPackageId(testPackageId);
        c4.setStatus(true);
        profileMapper.insert(c4);

        String code4 = profileService.generateCode(testPackageId);
