...
- `套餐/父套餐` 优先按系统父套餐名称或编码匹配；未填写或未匹配成功时，会从 `餐别` 长文本中兜底匹配已启用父套餐名称或编码，例如 `餐别：小月子餐、两荤一素一汤` 可匹配父套餐 `小月子餐`。
...
- 套餐优先接受系统父套餐名称或编码；`餐别` 中包含已启用父套餐名称或编码时可兜底映射父套餐，但 `不含汤/两荤一素` 等规格描述不会直接映射父套餐。
