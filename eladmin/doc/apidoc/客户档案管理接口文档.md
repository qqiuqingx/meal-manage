...
| orderInfo.mainDishCount | int | 否 | 每餐主菜/荤菜数量（null默认0，负数或超过10报错） |
| orderInfo.sideDishCount | int | 否 | 每餐副菜数量（null默认0，负数或超过10报错） |
| orderInfo.vegCount | int | 否 | 每餐素菜数量（null默认0，负数或超过10报错） |
| orderInfo.riceCount | int | 否 | 每餐米饭数量（null默认1，负数或超过10报错） |
| orderInfo.riceType | string | 否 | 米饭类型（默认`白米饭`） |
| orderInfo.soupCount | int | 否 | 每餐汤数量（null默认0，负数或超过10报错） |
